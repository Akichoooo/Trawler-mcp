...
源码位置：[`trawler/fetcher/curlcffi_rung.py`](trawler/fetcher/curlcffi_rung.py)
...
- **Accept-Encoding 原生解压**: [`curlcffi_rung.py:L126`](trawler/fetcher/curlcffi_rung.py#L126) 注入了 `Accept-Encoding: gzip, deflate, br, zstd`，降低了传输带宽，并使得请求头部顺序严格符合 Chrome 浏览器的排列。
...
### 2.1 基于数学的拟人轨迹算法
源码位置：[`trawler/fetcher/patchright_rung.py:L268`](trawler/fetcher/patchright_rung.py#L268) (`_inject_bezier_movement`)
...
源码位置：[`trawler/ssrf.py:L57`](trawler/ssrf.py#L57)
...
- **防重定向**: 对于 301/302 重定向（[`curlcffi_rung.py:L218`](trawler/fetcher/curlcffi_rung.py#L218)），由于 `curl_cffi` 自身的 `allow_redirects=True` 可能会绕开 DNS 预检，重构后彻底关闭了底层库的自动跳转，改为 **Python 手动接管循环重定向**，并在每一次 `Location` 解析时重新调用 `ssrf.check_url_safe()`。

### 3.2 浏览器沙箱层的网络阻断
源码位置：[`trawler/fetcher/patchright_rung.py:L142`](trawler/fetcher/patchright_rung.py#L142) 和 [`hitl_rung.py`](trawler/fetcher/hitl_rung.py)
...
在 [`patchright_rung.py:L380`](trawler/fetcher/patchright_rung.py#L380) (位于 `_get_context` 内部逻辑)，系统根据三元组 `(id(browser), proxy_server, storage_state_path)` 将构建好的 Context 缓存起来。
为了防止缓存带来的状态污染：
...
并非所有网站都走统一策略。[`trawler/site_rules.py`](trawler/site_rules.py) 提供了强大的领域特定降维打击能力。
...
### 6.1 拦截与防注入逻辑
源码位置：[`trawler/parser/safety.py`](trawler/parser/safety.py)
...
- **词库文件**：[`data/sensitive_words.txt`](data/sensitive_words.txt)
...
### 6.4 安全过滤挂载点
- **HTML DOM 前置清洗**：在 [`trawler/parser/extract.py:L43`](trawler/parser/extract.py#L43) 调用 `safety.sanitize_html_soup()`，移除含有 `display:none`、`visibility:hidden` 以及 `font-size:0px` 等隐藏样式的元素，防止其在背景中注入隐藏指令。
- **Markdown 最终净化**：在 [`trawler/parser/extract.py:L58`](trawler/parser/extract.py#L58) 返回最终文本前，对其整体应用 `safety.sanitize_markdown()` 掩码过滤。
